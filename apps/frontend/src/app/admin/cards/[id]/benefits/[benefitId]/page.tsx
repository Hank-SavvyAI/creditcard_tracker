...
export default function EditBenefitPage() {
  ...
  const benefitId = parseInt(params.benefitId as string)
  ...
  async function loadBenefit() {
    try {
      ...
      if (card) {
        const foundBenefit = card.benefits.find((b: any) => b.id === benefitId)
        if (foundBenefit) {
          setBenefit(foundBenefit)
          setFormData({
            category: foundBenefit.category || '',
            categoryEn: foundBenefit.categoryEn || '',
            title: foundBenefit.title || '',
            titleEn: foundBenefit.titleEn || '',
            description: foundBenefit.description || '',
            descriptionEn: foundBenefit.descriptionEn || '',
            amount: foundBenefit.amount ? String(foundBenefit.amount) : '',
            currency: foundBenefit.currency || 'TWD',
            frequency: foundBenefit.frequency || 'YEARLY',
            startMonth: foundBenefit.startMonth ? String(foundBenefit.startMonth) : '',
            startDay: foundBenefit.startDay ? String(foundBenefit.startDay) : '',
            endMonth: foundBenefit.endMonth ? String(foundBenefit.endMonth) : '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await api.updateBenefit(benefitId, data)
      alert('更新成功！')
  ...
  if (!benefit) {
    return <div className="dashboard">找不到此福利</div>
  }
